...
Here is a summary of the full dataset we used in our analyses (table \ref{tab:full-dataset}).
...
Table \ref{tab:compact-priors} provides a summary of the priors we specified for each term in our models. 

```{r compact-priors, echo = F}
data.frame(Prior_distribution = c("Normal(mean = 0, SD = 2)","Normal(mean = 1, SD = 2)"),
           Model_term = c("type_M.vs.A; subtype_Herb.vs.Para; subtype_Poll.vs.Disp; sc.r_ND:sc.c_ND; type_M.vs.A:sc.r_ND; type_M.vs.A:sc.c_ND; type_M.vs.A:sc.r_ND:sc.c_ND","sc.r_ND; sc.c_ND")) %>%
  kable(., caption = "Summary of prior distributions.", booktabs = T) %>%
  kableExtra::column_spec(column = 2, width = "6cm") %>%
  kable_styling(latex_options = c("striped", "hold_position"))
```


```{r set priors, echo=FALSE}
...
```{r plot-NDxND, echo=F, fig.cap="\\label{fig:plot-NDxND}Plot of how the sign of the statistical interaction between resource and consumer normalized degree (b_sc.r_ND:sc.c_ND) determines how symmetry (right panel) or asymmetry (left panel) in partner normalized degree affects partner fidelity.", warning=FALSE}
...
```{r nopriors-table-plot, echo=FALSE, fig.cap = "Comparing mean and 95\\% credible intervals of fixed effects from our full model with default vs. regularizing priors.", out.width="75%", fig.align="center"}
...
bind_rows(mutate(tidy_nopriors.brm, Prior_type = "Default"),
          mutate(tidy_full.brm, Prior_type = "Regularizing")) %>%
  filter(term.ord != "Intercept") %>%
  ggplot(., aes(x = term.ord, y = estimate, color = Prior_type)) +
  geom_linerange(aes(ymin = lower, ymax = upper), position = position_dodge(width = 0.5)) +
  geom_point(position = position_dodge(width = 0.5)) +
  coord_flip() +
  geom_hline(yintercept = 0, linetype = "dotted") +
  scale_color_manual(values = c("black","grey"), name = "Prior type") +
  ylab("Coefficient estimate") +
  xlab("")
```
...
```{r nopriors-table-random-plot, echo=FALSE, fig.cap = "Comparing mean and 95\\% credible intervals of random effects from our full model with default vs. regularizing priors.", out.width = "75%", fig.align="center"}
...
bind_rows(mutate(tidy_nopriors_brm.random, Prior_type = "Default"),
          mutate(tidy_full.brm.random, Prior_type = "Regularizing")) %>%
  ggplot(., aes(x = term.ord, y = estimate, color = Prior_type)) +
  geom_linerange(aes(ymin = lower, ymax = upper), position = position_dodge(width = 0.5)) +
  geom_point(position = position_dodge(width = 0.5)) +
  coord_flip() +
