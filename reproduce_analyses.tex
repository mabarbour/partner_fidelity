...
Here is a summary of the full dataset we used in our analyses (table
...
Table \ref{tab:compact-priors} provides a summary of the priors we
specified for each term in our models.

\begin{table}[!h]

\caption{\label{tab:compact-priors}Summary of prior distributions.}
\centering
\begin{tabular}[t]{l>{\raggedright\arraybackslash}p{6cm}}
\toprule
Prior\_distribution & Model\_term\\
\midrule
\rowcolor{gray!6}  Normal(mean = 0, SD = 2) & type\_M.vs.A; subtype\_Herb.vs.Para; subtype\_Poll.vs.Disp; sc.r\_ND:sc.c\_ND; type\_M.vs.A:sc.r\_ND; type\_M.vs.A:sc.c\_ND; type\_M.vs.A:sc.r\_ND:sc.c\_ND\\
Normal(mean = 1, SD = 2) & sc.r\_ND; sc.c\_ND\\
\bottomrule
\end{tabular}
\end{table}
...
interaction between resource and consumer normalized degree
(b\_sc.r\_ND:sc.c\_ND) determines how symmetry (right panel) or
asymmetry (left panel) in partner normalized degree affects partner
fidelity.}
\end{figure}
...
## R2 0.4009361 0.006391404 0.3883897 0.4134291
\end{verbatim}
...
\item
  ...
  interaction types equals 0.66(\(=\frac{\exp(\beta)}{\exp(\beta)+1}\),
  where \(\beta=\) 0.64) for the average level of normalized degree for
...
\item
  Pollination increases the log odds of partner fidelity by 2.59
  relative to seed dispersers.
\item
...
\item
  ...
  0.47 (\(=\frac{\exp(\beta)}{\exp(\beta)+1}\), where \(\beta=\) -0.13)
...
Note that the effect of \textbf{type\_M.vs.A} is higher in this subset
of data (fig. \ref{fig:weighted-table-plot}). This is likely due to the
fact that herbivory had a tendency to increase partner fidelity
(\textbf{subtype\_Herb.vs.Para} in fig. \ref{fig:full-table-plot});
therefore, its absence from the data subset enhances the the apparent
effect of mutualistic interactions. Other terms are similar, except that
there is less clear evidence of mutualistic interactions enhancing the
effect of resource normalized degree (\textbf{type\_M.vs.A:sc.r\_ND} in
fig. \ref{fig:weighted-table-plot}), and less clear evidence of symmetry
in partner normalized degrees to positively affect partner fidelity
(\textbf{sc.r\_ND:sc.c\_ND} in fig. \ref{fig:weighted-table-plot}).
Importantly, there is still clear evidence that mutualistic interactions
generally enhance partner fidelity (\textbf{type\_M.vs.A} in fig.
\ref{fig:weighted-table-plot}) and also enhance the effects of
...
\StringTok{    }\KeywordTok{filter}\NormalTok{(network_id }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\KeywordTok{as.character}\NormalTok{(M_networks}\OperatorTok{$}\NormalTok{network_id), }
...
only 53\% of the subsamples. This indicates that variation in sampling
...
\caption{Comparing mean and 95\% credible intervals of fixed effects from our full model with default vs. regularizing priors.}\label{fig:nopriors-table-plot}
\end{figure}
...
\caption{Comparing mean and 95\% credible intervals of random effects from our full model with default vs. regularizing priors.}\label{fig:nopriors-table-random-plot}
\end{figure}
